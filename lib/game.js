    ...
    _buildTicTacMessage(){
        ...
        for(var i = 0; i < this._board.length; i++) {
            ...
            for(var j = 0; j < this._board[i].length; j++) {
                ...
                if(this._board[i][j] != ':o:' || this._board[i][j]!= ':heavy_multiplication_x:'){
                    console.log("zapppp");
                    ticTacInterfaces.blocks[i].elements[j].text.text = ":zap:";
                }else{
                    console.log("pappppp");
                    ticTacInterfaces.blocks[i].elements[j].text.text = `${constants.P1SYMBOL}`;
                }
                var value = this._mapCoordstoMove(i,j);
                var actionId = this._mapCoordstoMove(i,j);
                ticTacInterfaces.blocks[i].elements[j].value = `${value}`;
                ticTacInterfaces.blocks[i].elements[j].action_id = `${actionId}`;
            }
        }
